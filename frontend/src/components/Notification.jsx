...
const Message = ({ initial, content, link, color, title, }) => {
  return (
    <div className="flex gap-4 py-4 border-b border-neutral-800 last:border-0">
      ...
      <Button variant="outline" size="sm" className="bg-neutral-800 border-neutral-700 text-white hover:bg-neutral-700" onClick={() => window.open(link, "_blank")}>
        Reply
      </Button>
    </div>
  );
...
export default function Notification() {
  const { reviews, loading, error, showMilestoneNotification, negativeReviews } = useFilterContext();
      ...
      <div className="relative z-10">
        ...
        <div className="mb-8">
          ...
          <Card className="bg-custom-gradient border border-[var(--color-bodcol)] z-10 text-white">
            ...
            <CardContent>
              {negativeReviews.length > 0 ? negativeReviews.map((review, index) => {
                return (
                  <Review
                    key={index}
                    author={review.name}
